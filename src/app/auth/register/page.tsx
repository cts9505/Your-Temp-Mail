      ...
      <div className="flex items-center justify-center min-h-screen pt-16 px-4 relative">
        <div className="w-full max-w-md border-4 md:border-[6px] border-black dark:border-white bg-white dark:bg-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] md:shadow-[24px_24px_0px_0px_rgba(0,0,0,1)] dark:shadow-[12px_12px_0px_0px_rgba(255,255,255,0.1)] md:dark:shadow-[24px_24px_0px_0px_rgba(255,255,255,0.1)] p-6 md:p-12">
          <div className="space-y-3 md:space-y-4 mb-6 md:mb-10 text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-black uppercase tracking-tighter italic">Join Us</h1>
          ...
          <form onSubmit={handleRegister} className="space-y-6 md:space-y-8">
            ...
            <div className="space-y-5 md:space-y-6">
              ...
              <div className="space-y-2">
                <label className="font-black text-[10px] md:text-xs uppercase tracking-[0.25em] md:tracking-[0.3em] block">Custom Alias (Optional)</label>
                <div className="relative group">
                  <AtSign className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 w-5 h-5 md:w-6 md:h-6 text-black dark:text-white" />
                  <Input
                    type="text"
                    placeholder="YOUR-ALIAS"
                    className="pl-12 md:pl-14 h-14 md:h-16 border-3 md:border-4 border-black dark:border-white rounded-none bg-white dark:bg-black font-black text-base md:text-lg focus-visible:ring-0 focus-visible:bg-zinc-50 dark:focus-visible:bg-zinc-950 transition-all placeholder:opacity-20 pr-32 md:pr-40"
                    value={alias}
                    onChange={(e) => setAlias(e.target.value.toLowerCase().replace(/[^a-z0-9]/g, ''))}
                  />
                  <div className="absolute right-3 md:right-4 top-1/2 -translate-y-1/2 font-black text-[10px] md:text-xs opacity-50 uppercase">
                    @{domain}
                  </div>
                </div>
              </div>
            ...
            <div className="text-center pt-3 md:pt-4">
              <Link href="/auth/login" className="font-black text-xs md:text-sm uppercase tracking-wider md:tracking-widest text-black dark:text-white hover:text-indigo-600 transition-colors border-b-3 md:border-b-4 border-indigo-600 pb-1">
